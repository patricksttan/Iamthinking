...
Fixpoint evalv (e:Expv) (l: list nat): nat := 
 match e with 
| Litv x => x
| StoreVar x y z => evalv z (store_var x (evalv y l) l) 
...
| Var x => (RPNvar x) :: nil
| StoreVar x y z => (rpnv y) ++ ((RPNvlit x)::nil) ++ (rpnv z) ++ ((RPNStore) :: nil)
     ...
     | RPNStore => 
         match l with 
          ...
          | x :: l2 =>
             match l2 with 
              | nil => None
              | y :: l3 => rpn_evalv1 l3 r2 (store_var x y k)
             end
        end
     | RPNvplus => 
         match l with 
